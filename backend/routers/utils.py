...
def derive_agents_from_tools(tools_used: Optional[List[str]]) -> List[str]:
    if not tools_used:
        return []
    # This mapping could be moved to a config or registry
    mapping = {
...
def assemble_memory(thread_id: int) -> List[Dict[str, str]]:
    ...
    max_msgs = int(os.getenv("UI_HISTORY_MAX_MSGS", "200"))
    tail_after_summary = int(os.getenv("UI_HISTORY_TAIL_AFTER_SUMMARY", "12"))
    boundary_context_msgs = int(os.getenv("UI_SUMMARY_BOUNDARY_CONTEXT", "3"))
    ...
    tail = [m for m in msgs if m.id > cutoff_id and m.role in ("system", "user", "assistant")]
...
def should_summarize(thread_id: int, upto_message_id: int, token_in: Optional[int], token_out: Optional[int]) -> bool:
    ...
    min_msgs = int(os.getenv("UI_SUMMARIZE_AFTER_MSGS", "12"))
    long_thread = int(os.getenv("UI_LONG_THREAD_MSGS", "20"))
    prob = float(os.getenv("UI_SUMMARIZE_PROB", "0.25"))
    token_trigger = int(os.getenv("UI_SUMMARIZE_TOKEN_TRIGGER", "2000"))
...
def generate_summary(thread_id: int, upto_message_id: int) -> Optional[str]:
    ...
    try:
        from core.settings import get_settings
        summary_model = getattr(get_settings(), "summarizer_model", None) or os.getenv("SUMMARIZER_MODEL")
        llm = get_llm_client(model=summary_model)
...
def count_user_messages(thread_id: int, since_message_id: Optional[int] = None) -> int:
    msgs = get_thread_messages(thread_id)
    msgs = [m for m in msgs if m.id > since_message_id] if since_message_id else msgs
...
def should_auto_rename_thread(thread_id: int) -> bool:
    ...
    rename_threshold = int(os.getenv("UI_THREAD_RENAME_THRESHOLD", "5"))
    return user_count >= rename_threshold

def get_last_rename_message_id(thread_id: int) -> Optional[int]:
    try:
        msgs = get_thread_messages(thread_id)
        return max((m.id for m in msgs if m.role == "assistant"), default=None) if len(msgs) >= 2 else None
    except Exception:
        return None
...
def summarize_assistant_response(content: str) -> str:
    ...
    truncated = [textwrap.shorten(line, width=150, placeholder="...") for line in lines[:8]]
    return "\n".join(truncated)
